...
| `fix/map-all-benchmark-lock` | fix | merged | `dev` | `dev` | Repin the canonical wrapper so Map All preserves compatible auxiliary streams and benchmark fallback remains authoritative. |
...
- Validation: 37 plugin unit tests, Python/JSON/shell checks, exact offline and remote wrapper verification, official Dispatcharr v0.29.0 notification-refresh and plugin-action UI contract review, workspace validation, feature/dev/tag GitHub workflows, immutable beta.10 archive inspection, development-registry validation/publication, public raw-manifest agreement, and `git diff --check` pass. Installed beta.10 validation remains pending.
- Publication state: immutable `v0.2.0-beta.10` is tagged and advertised through `dispatcharr-plugins:dev`; beta.9 remains immutable in registry history. No GitHub Release, stable-registry change, or distributable ZIP is authorized.
...
- Result: source commit `1e8e56f` merged into `dev` at `80c40ea164e5711dfbc37e8c465e943b9e1ee9ea`; immutable tag `v0.2.0-beta.11` resolves to that reviewed integration commit and is published through `dispatcharr-plugins:dev` at registry commit `34e2c61a6611b525d10a5eb89180b9915311a95e9`.
- Validation: 39 plugin tests, Python compilation, JSON parsing, shell syntax, exact offline and remote canonical-source verification, idempotent synchronization, workspace reconciliation, version/schema parity, immutable archive inspection, complete-diff review, and `git diff --check` pass. Source workflow `33024879688`, dev workflow `33024916207`, tag workflow `33024939375`, registry workflow `33025150782`, and public raw-manifest verification pass. Dispatcharr installed beta.11 with both bundled scripts executable and the exact beta.7 wrapper checksum. A full service-user recheck retained its owner lock, provided live stream-copy fallback with a persistent notification, completed at capacities 18 and 15, and cleared the notification. Map All updated without restart and produced a valid four-stream MPEG-TS capture containing one HEVC video, two copied AAC tracks, and the copied DVB subtitle; the original settings and managed profile were restored afterward.
- Started: `2026-08-26`.
