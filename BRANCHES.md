...
| `feature/session-completion-remote-checkpoint` | governance | active | `dev` at `093d1f2` | `dev` | Reconcile the mandatory session-end GitHub checkpoint rule without changing plugin behavior or distribution. |
...
- Validation: workspace standards validation and `git diff --check` pass; no plugin, wrapper, version, registry, release, or deployment behavior changed.
- Started: `2026-08-26`.
