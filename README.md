...
The plugin asks the canonical wrapper to validate the cache rather than treating an existing file as healthy. A missing, invalid, stale, or unverifiable cache creates a persistent, admin-only **FFmpeg Smart hardware scan required** warning in Dispatcharr's notification center. Starting a rebuild changes the same notification to **hardware scan in progress**; successful validation removes it automatically. This is the same database-backed notification mechanism Dispatcharr uses for account-expiration warnings, not a one-time toast.
...
Dispatcharr v0.29.0 currently treats a new viewer request rejected by the benchmark lock as an ordinary stream startup failure and may retry alternate streams. Its plugin event surface has no pre-start veto that can return maintenance cleanly. The persistent scan-in-progress notification makes the cause visible, but avoiding retry/failover and returning a maintenance response requires Dispatcharr core support for FFmpeg Smart's reserved exit status 75.
